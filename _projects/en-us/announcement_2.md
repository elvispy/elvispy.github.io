...
Large HEP pipelines launch thousands of short Julia jobs across heterogeneous nodes. Each cold start triggers JIT compilation and package precompilation from scratch, wasting CPU time and causing unpredictable tail latency. Sites also require read-only, content-addressed distribution (CernVM-FS, served via CVMFS), which rules out writable package depots.

The workflow builds, signs, and publishes precompiled Julia system images and package artifacts to CVMFS, then hydrates per-node depots on demand. The core tool, `DepotDelivery.jl`, orchestrates artifact bundling, version pinning, and cache layout. Artifacts are content-addressed (hash-stable) so nodes fetch identical byte streams regardless of site. A small set of micro-architecture images (e.g., baseline `x86-64`, `x86-64-v3`) covers the hardware diversity of the grid and is selected at runtime. On representative HEP stacks (jet reconstruction, Geant4 wrappers), cold-start latency dropped by an order of magnitude; nodes mount precompiled images with no write access and no local builds.
