...
A compact raft has no propeller, no fin, and no steady push. Yet move its vibrating motor away from the centre and the waves it radiates no longer leave equally in both directions. The wake becomes asymmetric; that asymmetric momentum flux gives the raft a direction of travel. The surprising part is that a zero-mean vibration can produce a nonzero mean thrust. It does so not by hiding a stroke cycle, but by changing how a deformable body launches waves into the surface.
