...
A thin native oxide on permalloy (Py) inflates FMR linewidths and biases the **Gilbert damping parameter** α upward. The effect is invisible unless linewidth slopes are tracked across many frequencies and surface chemistry is cross-checked by XPS. Extracting the spin Hall angle of the adjacent heavy-metal layer via the **inverse spin Hall effect (iSHE)** is further complicated by anisotropic magnetoresistance (AMR) of the ferromagnet. AMR generates a voltage at the same resonance condition, and geometry alone cannot separate it from the iSHE signal.
