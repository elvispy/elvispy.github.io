...
The KdV equation $u_t + 6u\,u_x + u_{xxx} = 0$ admits a self-similar solution where the relevant combination of variables is $\eta = x\,t^{-1/3}$ and the amplitude decays as $t^{-2/3}$. Those exponents follow from requiring each term in the PDE to scale identically. The requirement reduces to a linear system in the scaling exponents. Solving it by hand means matching terms one by one, and the algebra is error-prone: valid reductions can go unnoticed.
